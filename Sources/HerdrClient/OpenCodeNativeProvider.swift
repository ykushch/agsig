...
public struct OpenCodeSessionMessage: Decodable, Sendable, Equatable {
    ...
    public var userText: String? {
        guard info.role == "user" else { return nil }
        let text = parts.compactMap { part -> String? in
            guard part.type == "text", part.synthetic != true,
                  part.ignored != true else { return nil }
...
public extension OpenCodeNativeRequesting {
    func lastUserPrompt(descriptor: OpenCodePaneDescriptor) async throws -> String? {
        nil
    }
}
...
public struct OpenCodeHTTPClient: OpenCodeNativeRequesting, Sendable {
    ...
    public func lastUserPrompt(
        descriptor: OpenCodePaneDescriptor
    ) async throws -> String? {
        let value = try await request(
            descriptor: descriptor,
            components: ["session", descriptor.sessionID, "message"],
            method: "GET", queryItems: [URLQueryItem(name: "limit", value: "100")])
        return try value.decode([OpenCodeSessionMessage].self)
            .reversed().compactMap(\.userText).first
    }
...
public struct OpenCodeNativeInteractionProvider: InteractionProviding, Sendable {
    ...
    public func interaction(paneID: String, agentID: String?,
        ...
        let userPrompt = try? await client.lastUserPrompt(descriptor: descriptor)
        var choices = [
