...
struct StateStoreTests {
    ...
    @Test("blocked elapsed time is stable across refreshes and clears on resolution")
    func blockedTimestamp() throws {
        let current = Date(timeIntervalSince1970: 1_000)
        let store = StateStore(now: { current })
        store.hydrate(try loadSnapshot())
        _ = store.apply(statusEvent("w1:p1", "blocked"))
        #expect(store.blockedSince["w1:p1"] == current)

        _ = store.apply(statusEvent("w1:p1", "blocked"))
        #expect(store.blockedSince["w1:p1"] == Date(timeIntervalSince1970: 1_000))

        _ = store.apply(statusEvent("w1:p1", "idle"))
        #expect(store.blockedSince["w1:p1"] == nil)
    }
