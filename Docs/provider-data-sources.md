...
The first implementation reads pending permissions from `GET /permission` and
revalidates the native request ID plus normalized fingerprint immediately before
posting `once`, `always`, or `reject`. It supports both the current
